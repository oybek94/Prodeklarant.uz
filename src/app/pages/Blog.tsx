...
const BLOG_IMAGES = [
  "https://images.unsplash.com/photo-1466692479798-4f32f60a3147?q=80&w=800&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1518843875459-e438adba4292?q=80&w=800&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1574484284002-9520c62fd34f?q=80&w=800&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1597362925123-7782d98666fc?q=80&w=800&auto=format&fit=crop"
];

const AUTHORS = ["Azizbek Rahimov", "Dilnoza Karimova", "Jamshid Aliyev", "Azizbek Rahimov"];

export function getBlogPosts(t: (key: string) => string) {
  return [1, 2, 3, 4].map((id, i) => ({
    id,
              ...
              {blogPosts.map((post) => (
                <div key={post.id} className="bg-white rounded-sm shadow-sm overflow-hidden flex flex-col md:flex-row hover:shadow-lg transition-shadow border border-slate-100">
                  ...
                  <div className="p-6 md:w-2/3 flex flex-col justify-between">
                    ...
                    <Link 
                      to={`/blog/${post.id}`} 
                      className="text-blue-900 font-bold text-sm uppercase flex items-center gap-2 hover:text-yellow-600 transition-colors self-start"
                    >
          ...
          <div className="lg:w-1/3 space-y-8">
            ...
            <div className="bg-white p-6 rounded-sm shadow-sm border border-slate-100">
              ...
              <div className="relative">
                ...
                />
                <button className="absolute right-3 top-3 text-slate-400 hover:text-blue-900">
                  <Search size={18} />
                </button>
              </div>
            </div>
            ...
            {/* Subscribe Widget */}
            <div className="bg-blue-900 p-6 rounded-sm shadow-sm text-white">
              <h3 className="text-lg font-bold mb-2 uppercase">{t('blog.subscribe')}</h3>
              <p className="text-blue-200 text-sm mb-4">{t('blog.subscribeDesc')}</p>
              <input 
                type="email" 
                placeholder={t('blog.subscribePlaceholder')} 
                className="w-full px-4 py-2 mb-3 text-slate-900 text-sm focus:outline-none"
              />
              <button className="w-full bg-yellow-500 hover:bg-yellow-600 text-slate-900 font-bold py-2 uppercase text-xs tracking-wider transition-colors">
                {t('blog.subscribeBtn')}
              </button>
            </div>
          </div>
